...
def to_nums(string):
    ...
    # Convert mixed fraction style like "7((2)/(3))" to "(7+2/3)".
    s = re.sub(r'(?:(?<=^)|(?<=[+\-*/(]))(\d+)\s*\(\((\d+)\)\s*/\s*\((\d+)\)\)', r'(\1+\2/\3)', s)
    # Convert mixed fraction style like "3(1/2)" to "(3+1/2)"
    s = re.sub(r'(?:(?<=^)|(?<=[+\-*/(]))(\d+)\((\d+)\s*/\s*(\d+)\)', r'(\1+\2/\3)', s)
    s = s.replace('%', '/100')
...
def prepare_data_23k(pairs_trained, pairs_tested, trim_min_count, generate_nums, copy_nums, tree=False):
    ...
    test_pairs = []
    test_pairs_ape = []
    tokenizer = BertTokenizer.from_pretrained(DEFAULT_ZH_BERT_PATH)
...
def _safe_change_num(num_list):
    result = []
    for item in num_list:
        if '/' in item:
            new_str = item.split(')')[0].split('(')[1]
            a = float(new_str.split('/')[0])
            b = float(new_str.split('/')[1])
            result.append(a / b)
        elif '%' in item:
            result.append(float(item[0:-1]) / 100)
        else:
            result.append(float(item))
    return result


def build_number_relation_graph(num_values):
    if not num_values:
        graph = np.zeros((5, 1, 1), dtype=np.float32)
        graph[:, 0, 0] = 1.0
        return graph

    num_size = len(num_values)
    graph = np.zeros((5, num_size, num_size), dtype=np.float32)
...
def get_single_batch_graph(num_value_batch):
    max_num_size = max(1, max(len(values) for values in num_value_batch))
    batch_graph = []
    for values in num_value_batch:
        base_graph = np.zeros((5, max_num_size, max_num_size), dtype=np.float32)
        local_graph = build_number_relation_graph(values)
        size = local_graph.shape[-1]
        base_graph[:, :size, :size] = local_graph
        for ridx in range(base_graph.shape[0]):
            for i in range(size, max_num_size):
                base_graph[ridx, i, i] = 1.0
        batch_graph.append(base_graph)
    return np.stack(batch_graph, axis=0)


def get_single_example_graph(num_value):
    return np.expand_dims(build_number_relation_graph(num_value), axis=0)


def _build_graph_pairs(raw_pairs, output_lang, input_lang, tokenizer, tree):
    pairs = []
    for pair in raw_pairs:
        num_stack = []
        input_tokens = ['n' if token == 'NUM' else token for token in pair[0]]
        for word in pair[1]:
            temp_num = []
            flag_not = True
            if word not in output_lang.index2word:
        ...
        inputs = tokenizer(input_tokens, is_split_into_words=True, return_tensors="pt", add_special_tokens=False)
        num_pos = []
        for idx, token_id in enumerate(inputs['input_ids'].squeeze()):
            if tokenizer.convert_ids_to_tokens(int(token_id)) == 'n':
                num_pos.append(idx)

        num_stack.reverse()
        input_cell = indexes_from_sentence(input_lang, input_tokens)
        output_cell = indexes_from_sentence(output_lang, pair[1], tree)
        if output_lang.word2index["UNK"] in output_cell:
            continue
        if len(input_cell) > 100 or len(output_cell) > 20:
            continue
        try:
            num_values = _safe_change_num(pair[2])
        except Exception:
            continue
        if len(num_values) != len(num_pos):
            continue
        pairs.append(
            (
                input_cell,
                inputs['input_ids'].squeeze().size(0),
                output_cell,
                len(output_cell),
                pair[2],
                num_pos,
...
def prepare_data_23k_graph(pairs_trained, pairs_tested, trim_min_count, generate_nums, copy_nums, tree=False):
    input_lang = Lang()
    output_lang = Lang()
    tokenizer = BertTokenizer.from_pretrained(DEFAULT_ZH_BERT_PATH)
    print("Indexing words for graph-enhanced pipeline...")
    ...
    train_pairs = _build_graph_pairs(pairs_trained, output_lang, input_lang, tokenizer, tree)
    test_pairs = _build_graph_pairs(pairs_tested, output_lang, input_lang, tokenizer, tree)
    print('Indexed %d words in input language, %d words in output' % (input_lang.n_words, output_lang.n_words))
    print('Number of graph training data %d' % (len(train_pairs)))
    print('Number of graph testing data %d' % (len(test_pairs)))
    return input_lang, output_lang, train_pairs, test_pairs
...
def prepare_data_rbt(pairs_trained, pairs_tested, pairs_tested_ape, trim_min_count, generate_nums, copy_nums, tree=False):
    ...
    test_pairs = []
    test_pairs_ape = []
    tokenizer = BertTokenizer.from_pretrained(DEFAULT_ZH_ROBERTA_PATH)
    print("Indexing words...")
    for pair in pairs_trained:
        if not tree:
            input_lang.add_sen_to_vocab(pair[0])
...
def prepare_train_batch_graph(pairs_to_batch, batch_size):
    pairs = copy.deepcopy(pairs_to_batch)
    ...
    for batch in batches:
        ...
        for i, li, j, lj, num, num_pos, num_stack, bert_input, num_values in batch:
            num_batch.append(len(num))
            input_batch.append(pad_seq(i, li, input_len_max))
            output_batch.append(pad_seq(j, lj, output_len_max))
            num_stack_batch.append(num_stack)
            ...
            num_size_batch.append(len(num_pos))
            bert_input_batch.append(bert_input)
            num_value_batch.append(num_values)
        ...
        num_size_batches.append(num_size_batch)
        bert_input_batches.append(bert_input_batch)
        graph_batches.append(get_single_batch_graph(num_value_batch))
