...
parser.add_argument("--save_dir", type=str, default="models_graph")
...
def main():
    ...
    if _is_primary():
        ...
        print(
            ...
            "relation_dropout=%.2f active_relations=%s ckpt_every=%d"
            % (
    ...
    if use_ddp:
        # BERT pooler parameters can stay unused in this training graph; keep DDP robust to unused params.
    ...
    for epoch in range(n_epochs):
        ...
        if should_eval:
            ...
            if _is_primary():
                ...
                print("test_answer_acc", equation_acc, value_acc)
                ...
                if is_better:
                    ...
                    print(
                        "[save] new_best epoch=%d equation_acc=%.6f value_acc=%.6f -> %s"
                        % (best_epoch, best_equation_acc, best_value_acc, args.save_dir)
                    )
                else:
                    print(
                        "[save] keep_best epoch=%d equation_acc=%.6f value_acc=%.6f"
    ...
    if _is_primary():
        if best_epoch > 0:
            print(
                "[best] epoch=%d equation_acc=%.6f value_acc=%.6f"
                % (best_epoch, best_equation_acc, best_value_acc)
            )
        else:
